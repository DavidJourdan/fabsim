...
public:
  ...
  double
  energy(const Eigen::Ref<const Eigen::VectorXd> X, const LocalFrame<double> &f1, const LocalFrame<double> &f2) const;
  LocalVector
  gradient(const Eigen::Ref<const Eigen::VectorXd> X, const LocalFrame<double> &f1, const LocalFrame<double> &f2) const;
  LocalMatrix
  hessian(const Eigen::Ref<const Eigen::VectorXd> X, const LocalFrame<double> &f1, const LocalFrame<double> &f2) const;

  double get_reference_twist() const { return _ref_twist; }
  void set_reference_twist(double ref_twist) { _ref_twist = ref_twist; }
  void update_reference_twist(const LocalFrame<double> &f1, const LocalFrame<double> &f2);
