...
TEMPLATE_TEST_CASE("HingeElement", "[HingeElem]", SquaredAngleFormulation, TanAngleFormulation)
{
  ...
  SECTION("Equivalence")
  {
    ...
    VectorXd X = GENERATE(take(2, vector_random(12)));
...
TEST_CASE("signed angle")
{
  ...
  Vector3d u = GENERATE(take(2, vector_random(3)));
  Vector3d axis = GENERATE(take(2, vector_random(3)));
  axis = u.cross(axis).normalized();
  double angle = GENERATE(take(2, random(-M_PI, M_PI)));
